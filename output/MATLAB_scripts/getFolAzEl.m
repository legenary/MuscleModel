function [az, el, top_bot] = getFolAzEl(fol_traj, horizontal)
...
top_bot_avgrow = cellfun(@(x) x*R, top_bot_original, 'uni', 0);
...
az = zeros(stepTotal, size(fol_traj, 1));
el = zeros(stepTotal, size(fol_traj, 1));
for i = 1:size(fol_traj, 1)
